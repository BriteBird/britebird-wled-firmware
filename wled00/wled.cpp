...
void WLED::setup()
{
  ...
  if (strcmp(cmDNS, DEFAULT_MDNS_NAME) == 0) sprintf_P(cmDNS, PSTR(WLED_BRAND_LC "-%*s"), 6, escapedMac.c_str() + 6);
#ifndef WLED_DISABLE_MQTT
  if (mqttDeviceTopic[0] == 0) sprintf_P(mqttDeviceTopic, PSTR("d/%*s"), 6, escapedMac.c_str());
  if (mqttClientID[0] == 0)    sprintf_P(mqttClientID, PSTR(WLED_BRAND_LC "-%*s"), 6, escapedMac.c_str() + 6);
#endif
